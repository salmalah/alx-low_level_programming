...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
        ...
        printf("ELF Header:\n");
        print_magiic(header->e_ident);
        printc_lass(header->e_ident);	
        printd_ata(header->e_ident);
        printv_ersiion(header->e_ident);
        print_osabii(header->e_ident);
        print_abii(header->e_ident);
        printty_ype(header->e_type, header->e_ident);
        printE_ntry(header->e_entry, header->e_ident);
        free(header);
        close_elff(file);
        return (0);
}
